...
\RCS$Id: SUS-14-022.tex 331161 2016-03-07 22:06:41Z alcaraz $
%%%%%%%%%%%%% local definitions %%%%%%%%%%%%%%%%%%%%%
...
\title{Search for electroweak production of charginos in final states with two tau leptons in pp collisions at $\sqrt{s}= 8~\mathrm{TeV}$}
...
\abstract{
A search for electroweak production of supersymmetric particles is performed with two $\tau$ leptons in the final state. 
These results are based on data of proton-proton collisions at $\sqrt{s} = 8~\mathrm{TeV}$, collected with the CMS detector at the LHC and corresponding to an integrated luminosity between 18.1 and $19.6~\mathrm{fb}^{-1}$. 
%These results are based on 18.1 to $19.6~\mathrm{fb}^{-1}$ of proton-proton collisions at $\sqrt{s} = 8~\mathrm{TeV}$, collected with the CMS detector at the LHC. 
The observed events are found to be consistent with the standard model background prediction. Upper limits are set on the masses of the lightest chargino and the lightest neutralino, assuming the third-generation sleptons are the lightest sleptons and their masses are midway between the chargino and neutralino masses. In the context of simplified model spectra, charginos lighter than $420~\mathrm{GeV}$ are excluded at a $95\%$ confidence level in the case of a massless neutralino.
}    
...
pdftitle={Search for electroweak production of charginos in final states with two $\tau$ leptons in pp collisions at $\sqrt{s}= 8~\mathrm{TeV}$},%
pdfsubject={CMS},%
