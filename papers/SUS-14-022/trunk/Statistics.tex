...
are based on the estimate described in Section \ref{sect:bkgFake} and labeled ``WX''.
...
A modified frequentist approach, known as the LHC-style CLs criterion \cite{read:CLs}, is used to 
set limits on cross sections at 95\% confidence level (C.L.).
...
The expected limits and the contours corresponding to $\pm 1$ standard deviations from experimental uncertainties are shown in red lines, respectively. 
...
Since the cross section for direct production of sleptons is lower, no point is excluded and a $95\%$ C.L. upper limit is set on 
