...
my $VERSION = sprintf "%d.%03d", q$Revision: 215631 $ =~ /(\d+)/g;
my $verbose;
...
#
   ...
   # \RCS$Revision: 215631 $
   # \RCS$Date: 2013-11-08 00:05:58 +0330 (Fri, 08 Nov 2013) $
   # \RCS$HeadURL: svn+ssh://svn.cern.ch/reps/tdr2/utils/trunk/general/makeManifest.pl $
   # \RCS$Id: makeManifest.pl 215631 2013-11-07 20:35:58Z alverson $
   ...
   if ($checkRefs)
   {
       ...
       { 
            my $status = system("$bin_dir/cleanRefs.py", $baseArg, $tag);
       }
       else 
       {
...
#      &insert_datafield("859",$record,'cds.support@cern.ch');
      ...
      if ($thumbnails) 
      {
          if ($^O eq "MSWin32")
          {
              ...
              $convertCmd = $ENV{'ProgramFiles'}."\\ImageMagick-6.8.7-Q16\\convert.exe";
          }
          else
          {
              $convertCmd = "convert";
