...
tight ($N_{Tight}$) and non-tight ($N_{NTight}$), so one can write:
\begin{equation}
...
The real rate ($r_{real}$) is measured in Monte Carlo Drell-Yan events, and it is found to 
...
$r_{fake}$ ($r_{real}$) is shorthand for fake (prompt) rate.}
