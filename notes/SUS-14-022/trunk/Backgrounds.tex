...
N_{\rm SR} = N_{\rm Before~Final~Selection} \times \epsilon_{FS}.
\end{equation}
Where $\epsilon_{FS}$ is $\epsilon_{ \rm M_{T2}}= \frac{N (\rm M_{T2}>90)}{N (\rm M_{T2}>40)}$ for \binone and $\epsilon_{\SumMT}=\frac{N (\SumMT>250)}{N(40<\rm M_{T2}<90)}$ for \bintwo and $N_{\rm Before~Final~Selection}$ is the number of \wjets events just before applying the final selection 
(\mttwo $>$ 90\GeV for \binone and \SumMT $>$ 250\GeV for \bintwo). %and $N_{\rm SR}$ is the final estimation of \wjets events in the signal region.
...
The distribution of invariant mass of ($\mu, \Tau$) system for data and Monte Carlo events are in a good agreement.
...
A separate method is developed to estimate the misidentified contamination in these channels in the following action.
