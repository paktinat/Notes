...
Otherwise data-to-MC scale factors are used to correct or to validate the expected contributions obtained from the 
simulated samples. The estimates of main backgrounds are discussed below.
...
All reported uncertainties are quadratic sum of the statistical and systematic uncertainties.
...
To evaluate the uncertainties due to correlation assumptions, different fit models are tested. For both transfer factor and 
\deltaphi efficiency, fitting the whole range by a horizontal line or a line with a constant slope or using the value of the last 
bin before entering the signal region are examined. The weighted average of the estimates is compared with the reported values 
in Table \ref{4QCDbg} to extract the uncertainty shown as ``Fit Unc'' in the table.
...
N_{\rm SR} = N_{\rm Before~Final~Selection} \times \epsilon_{FS}.
\end{equation}
Here $\epsilon_{FS}$ is $\epsilon_{ \rm M_{T2}}= \frac{N (\rm M_{T2}>90)}{N (\rm M_{T2}>40)}$ for \binone or  $\epsilon_{\SumMT}=\frac{N (\SumMT>250)}{N(40<\rm M_{T2}<90)}$ for \bintwo and $N_{\rm SR}$ is the final estimation of \wjets events in the signal region. $N_{\rm Before~Final~Selection}$ is the number of \wjets events just before applying the final selection 
(\mttwo $>$ 90\GeV for \binone and \SumMT $>$ 250\GeV for \bintwo), which is 31.93$\pm$6.40 and 29.13$\pm$6.22 for \binone and
\bintwo, respectively. The uncertainties are Monte Carlo statistics. %and $N_{\rm SR}$ is the final estimation of \wjets events in the signal region.

%To improve the statistics of Monte Carlo samples, we relax some kinematic selections.
The $\epsilon_{FS}$ is first evaluated in a \wjets Monte Carlo sample with a pair of opposite-sign \Tau where the \Tau candidates are selected similar to those in signal region, but with a loose kinematic selection to improve statistics.
...
The \wjets Monte Carlo sample is validated in data using a same-sign \muTau control sample, where both the normalization and $\epsilon_{FS}$ are checked. 
...
for \binone(\bintwo) and its uncertainty is also taken to be the systematic uncertainty and referred to as ``Shape Unc''.
...
variation of the estimation found  from varying the \Tau energy scale within its uncertainty.
 The ``Shape Unc'' takes into account the difference between the shape of the search variable distribution in data and Monte Carlo.}
\begin{tabular}{|l|c|}
\hline\hline
Signal Region & \wjets Estimation Results\\
\hline
\tauTau \binone & 0.72 $\pm$ 0.11 (Stat) $\pm$ 0.11 (Sys) $\pm$ 0.56 (Shape Unc)\\
\tauTau \bintwo & 2.58 $\pm$ 0.35 (Stat) $\pm$ 1.04 (Sys) $\pm$ 0.69 (Shape Unc)\\
\hline\hline
\end{tabular}
\label{tbl:Wbkg}
\end{center}
...
The distributions of invariant mass of \muTau system for data and Monte Carlo events are in a good agreement.
...
When the loose signal selection is applied, the number of loose \Tau objects ($N_{\rm Loose}$) is:
...
\begin{equation}
   N_{\rm Fake}\times (r_{\rm Fake}-r_{\rm Real}) = ((1 - r_{\rm Real})\times N_{\rm Loose}  - N_{\rm Tight})
\end{equation}
Here $r_{\rm Fake}\times N_{\rm Fake}$ is the contamination of misidentified \Tau objects to the signal region. 

The fake rate ($r_{\rm Fake}$) is measured as the ratio of tightly selected \Tau objects to loosely 
selected \Tau objects in a sample which is dominated by misidentified \Tau objects. This is done in a sample of same-sign \leptonTau selected 
with the same requirements as the opposite-sign \leptonTau
...
As a cross check, the fake rate is also measured in an opposite sign region with a reversed
\MPT requirement, i.e., \MPT $<$ 30\GeV.
The result is consistent with $r_{\rm Fake}$ = 0.51. 
...
A conservative relative systematic uncertainty of 5\% is assigned to the central values of $r_{\rm Real}$ and $r_{\rm Fake}$.
...
It correctly
predicts the number of \leptonTau background events in this sample, within the 
uncertainties.
