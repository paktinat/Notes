...
and scaling by the transition factor to go from loosely isolated SS to tightly isolated opposite-sign (OS) 
...
which is compatible with 1.0, within the uncertainties. 
For $\epsilon_{FS}$, 
...
is very small, because the misidentification probability for $\ell\rightarrow$  \Tau 
is sufficiently low.  The misidentification probability for  \Tau $\rightarrow\ell$ is also low, so the probability 
