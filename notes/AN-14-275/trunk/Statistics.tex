...
\begin{figure}[h]
\centering
%\includegraphics[width=0.45\textwidth,keepaspectratio=true]{StatisticsFig/QCDWestimation_plot.png}
%\includegraphics[width=0.45\textwidth,keepaspectratio=true]{StatisticsFig/QCDestimation_plot.png}
...
as ``W'' which was described in Sec.\ref{sect:bkgFake}. The QCD multijet contribution is very low for these channels and is counted among ``W''.
...
different signal regions of \tauTau channel are shown. The QCD multijet contribution in those plots come from the data driven method described in 
Sec.\ref{sect:bkgQCD}. The \wjets in the last bin of two plots was described in Sec.\ref{sect:bkgW}. 

Backgrounds are taken into account in different categories, including DY which is Monte-Carlo-Driven, Low rate backgrounds (\ttbar, diboson ``VV'', Higgs boson), 
``W'' and QCD which are data driven.
...
2.95, 7.27, 1.60 and 5.29, respectively.
DY yields are feed to the package as a Gamma distribution with the corresponding statistical weights for each bin.
%Furthermore, 25\% and 28\% systematic uncertainty on DY backgrounds are also considered. 
All systematics are taken through LogNormal distributions. For the Low rate backgrounds, a flat 50\% uncertainty is assigned.
The shape-altering systematic uncertainties are considered correlated for two signal regions of the \tauTau channel. 
...
The \wjets background estimation of two signal regions of the \tauTau channel are also considered fully correlated. 
The systematic uncertainties for the fake estimation in $\ell\Tau$ channels are also 100\% correlated. 
The other systematics are treated unclorrelated 
%20\% and 25\% systematic uncertainty on signal yields are considered. 
...
The \sTau searches in the LEP experiments \cite{lepsusy} have excluded the masses below 110 \GeV. In Fig.~\ref{fig:limit_final}, 
...
The results of the \tauTau channels are interpreted to set limit on the  \sTau\sTau production, which corresponds to the right diagram in Fig.~\ref{fig:Productions}. In this simplified model, two \sTau are directly produced from the $pp$ collision and decay instantly into two $\tau$ and two \PSGczDo. To calculate the production cross section, \sTau is 
defined as a maximal admixture of the left-handed and right-handed \sTau gauge-eigenstates \cite{Fuks:2013lya}.
As the cross section of direct production of sleptons is lower,
\begin{linenomath}
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth,keepaspectratio=true]{StatisticsFig/ExclusionSTauSTauLsp1.pdf}
\caption{The exclusion power of the \tauTau channels in $\sTau\sTau$ production.}
...
The best limit which corresponds to the lowest signal strength is obtained for $m_{\sTau}=110 \GeV$. The observed (expected) upper limit on the cross section at this mass is 228 (328) fb which is almost three times larger than the theoretical NLO cross section.
