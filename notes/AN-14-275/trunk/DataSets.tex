...
\hline
\multicolumn{3}{|c|}{$e\Tau$ and $\mu\Tau$ channels} \\
\hline\hline
...
For data the golden JSON file, {\small Cert\_190456-208686\_8TeV\_22Jan2013ReReco\_Collisions12\_JSON.txt} is used. In Table.~\ref{Tab.Triggers} shows the list of trigger paths which are used for this analysis. 
\begin{table}[!Hhtb]
\begin{center}
\small{
\caption{
  Trigger paths used by $e\Tau$, $\mu\Tau$ and $\tauTau$ channels
...
\hline\hline
\multicolumn{2}{|c|}{QCD used in $e/\mu\Tau$ channels }\\
\hline\hline
%QCD BCtoE                        &    
...
\hline\hline
\multicolumn{2}{|c|}{QCD used in $\tauTau$ channel }\\
\hline\hline
%QCD                                &   
...
The investigated SUSY signal in this analysis is {\small $pp \rightarrow \chip \chim \rightarrow \tau^+ \tau^- \MPT$} called {\small "TChipChimSlepSnu"} produced with \PYTHIA $6.4$ ~\cite{PYTHIA} at the level of LHE. \TAUOLA 1.1.1a~\cite{Davidson:2010rw} is used to decay precisely the produced $\tau$'s, then the generated events are passed through the CMS official Fast-Sim process.
